...
> This package is pre-1.0, unaudited, and the API may change without notice. It manages cryptographic keys and submits real on-chain transactions. Do not use it with funds or workloads you can't afford to lose. No warranty is provided — see [LICENSE](./LICENSE).
...
```bash
pnpm add -g @fesal-packages/odws
odws wallet list
```
...
```typescript
import { IkaOWSProvider, derivePrivateKeyFromMnemonic, bytesToHex } from '@fesal-packages/odws';
...
```typescript
import { generateMnemonic, derivePrivateKeyFromMnemonic, bytesToHex } from '@fesal-packages/odws';
import { Curve } from '@ika.xyz/sdk';
...
```typescript
import { exportVault, importVault } from '@fesal-packages/odws';
